...
pandora::StatusCode DDMCParticleCreator::CreateMCParticles(const EVENT::LCEvent* const pLCEvent) const {
  for (StringVector::const_iterator iter = m_settings.m_mcParticleCollections.begin(),
    ...
    try {
      ...
      for (int i = 0, iMax = pMCParticleCollection->getNumberOfElements(); i < iMax; ++i) {
        try {
          ...
          mcParticleParameters.m_endpoint = pandora::CartesianVector(
              pMcParticle->getEndpoint()[0], pMcParticle->getEndpoint()[1], pMcParticle->getEndpoint()[2]);
      ...
      streamlog_out(DEBUG5) << "Failed to extract MCParticles collection: " << *iter << ", " << exception.what()
                            << std::endl;
    }
  }
...
pandora::StatusCode DDMCParticleCreator::CreateTrackToMCParticleRelationships(const EVENT::LCEvent* const pLCEvent,
                                                                              const TrackVector& trackVector) const {
  for (StringVector::const_iterator iter = m_settings.m_lcTrackRelationCollections.begin(),
                                    iterEnd = m_settings.m_lcTrackRelationCollections.end();
       iter != iterEnd; ++iter) {
    try {
      ...
      for (TrackVector::const_iterator trackIter = trackVector.begin(), trackIterEnd = trackVector.end();
        ...
        try {
          ...
          const pandora::Helix helixFit(pTrack->getPhi(), pTrack->getD0(), pTrack->getZ0(), pTrack->getOmega(),
                                        pTrack->getTanLambda(), m_bField);
          ...
          for (EVENT::LCObjectVec::const_iterator itRel = objectVec.begin(), itRelEnd = objectVec.end();
               itRel != itRelEnd; ++itRel) {
            ...
            const float trueMomentum(pandora::CartesianVector(pMCParticle->getMomentum()[0],
                                                              pMCParticle->getMomentum()[1],
          ...
          streamlog_out(WARNING) << "Failed to extract track to mc particle relationship: " << exception.what()
                                 << std::endl;
        }
      }
    } catch (EVENT::Exception& exception) {
      streamlog_out(DEBUG5) << "Failed to extract track to mc particle relationships collection: " << *iter << ", "
...
pandora::StatusCode
DDMCParticleCreator::CreateCaloHitToMCParticleRelationships(const EVENT::LCEvent* const pLCEvent,
  ...
  for (StringVector::const_iterator iter = m_settings.m_lcCaloHitRelationCollections.begin(),
    ...
    try {
      ...
      for (CalorimeterHitVector::const_iterator caloHitIter = calorimeterHitVector.begin(),
                                                caloHitIterEnd = calorimeterHitVector.end();
          ...
          for (MCParticleToEnergyWeightMap::const_iterator mcParticleIter = mcParticleToEnergyWeightMap.begin(),
            ...
            PANDORA_THROW_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=,
                                    PandoraApi::SetCaloHitToMCParticleRelationship(
                                        m_pandora, *caloHitIter, mcParticleIter->first, mcParticleIter->second));
          }
        } catch (pandora::StatusCodeException& statusCodeException) {
...
DDMCParticleCreator::Settings::Settings()
    : m_mcParticleCollections(StringVector()), m_lcCaloHitRelationCollections(StringVector()),
