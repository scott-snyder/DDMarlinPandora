...

{
  ...
  layers: " << N<< " sum up to now: "<<m_barrelTrackerLayers<<std::endl; } catch (std::runtime_error &exception){
          streamlog_out(WARNING) << "DDTrackCreatorALLEGRO exception during Barrel Tracker layer sum for
  ...
  " << N<< std::endl;
          ...
          streamlog_out(WARNING) << "DDTrackCreatorALLEGRO exception during Forward Tracking Disk parameter construction
  for detector "<<const_cast<dd4hep::DetElement&>(*iter).name()<<" : " << exception.what() << std::endl;
      }
  }
...
bool DDTrackCreatorALLEGRO::PassesQualityCuts(const EVENT::Track* const pTrack,
...
void DDTrackCreatorALLEGRO::DefineTrackPfoUsage(const EVENT::Track* const pTrack,
...
void DDTrackCreatorALLEGRO::TrackReachesECAL(const EVENT::Track* const pTrack,
                                             PandoraApi::Track::Parameters& trackParameters) const {
