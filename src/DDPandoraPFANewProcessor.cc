...
pandora::StatusCode DDPandoraPFANewProcessor::RegisterUserComponents() const {
  ...
  if (m_settings.m_useAPRIL) {
    ...
    PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=,
                             APRILContent::RegisterAPRILPseudoLayerPlugin(*m_pPandora));
    PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, APRILContent::RegisterParticleIds(*m_pPandora));

    PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=,
                             APRILContent::RegisterAPRILShowerProfilePlugin(*m_pPandora));

    PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=,
                             APRILLCContentPluginRegistration(*m_pPandora)); // Register LCContent plugins
  }
#endif
...
void DDPandoraPFANewProcessor::ProcessSteeringFile() {
  ...
  registerProcessorParameter("ECALInputEnergyCorrectionPoints",
                             ...
                             m_settings.m_ecalInputEnergyCorrectionPoints, FloatVector());
  ...
  registerProcessorParameter("ECALOutputEnergyCorrectionPoints",
                             ...
                             m_settings.m_ecalOutputEnergyCorrectionPoints, FloatVector());
  ...
  registerProcessorParameter("DetectorName", "The name of the detector", m_settings.m_detectorName, std::string(""));
  ...
  registerProcessorParameter("UseAPRIL", "Whether to use APRIL instead of Pandora for the reconstruction",
                             m_settings.m_useAPRIL, false);

  registerProcessorParameter("UseAPRIL_Calo", "Whether to use APRIL instead of Pandora for the reconstruction",
                             m_caloHitCreatorSettings.m_useAPRIL, false);
}
...
//------------------------------------------------------------------------------------------------------------------------------------------
// Added by T.Pasquier
// Temporary method that allows the registration of LCContent corrections and pID plugins without registering
// PseudoLayerPlugin or ShowerProfilePlugin Would be better to modify the plugins registration directly in LCContent
pandora::StatusCode APRILLCContentPluginRegistration(const pandora::Pandora& pandora) {
  PandoraApi::RegisterEnergyCorrectionPlugin(pandora, "CleanClusters", pandora::HADRONIC,
                                             new lc_content::LCEnergyCorrectionPlugins::CleanCluster);
  PandoraApi::RegisterEnergyCorrectionPlugin(pandora, "ScaleHotHadrons", pandora::HADRONIC,
                                             new lc_content::LCEnergyCorrectionPlugins::ScaleHotHadrons);
  PandoraApi::RegisterEnergyCorrectionPlugin(pandora, "MuonCoilCorrection", pandora::HADRONIC,
