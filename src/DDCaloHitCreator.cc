...
DDCaloHitCreator::DDCaloHitCreator(const Settings& settings, const pandora::Pandora* const pPandora)
    : m_settings(settings), m_pandora(*pPandora), m_hCalBarrelLayerThickness(0.f), m_hCalEndCapLayerThickness(0.f),
      m_calorimeterHitVector(0), m_volumeManager(), m_caloHitFactory() {
    ...
    try {
      ...
      for (int i = 0; i < nElements; ++i) {
        try {
          ...
          // ATTN If using strip splitting, must correct cell sizes for use in PFA to minimum of strip width and strip
          // length
          ...
          PANDORA_THROW_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=,
                                  PandoraApi::CaloHit::Create(m_pandora, caloHitParameters, *m_caloHitFactory));
...
pandora::StatusCode DDCaloHitCreator::CreateMuonCaloHits(const EVENT::LCEvent* const pLCEvent) {
  for (StringVector::const_iterator iter = m_settings.m_muonCaloHitCollections.begin(),
    ...
    try {
      ...
      // const std::vector<dd4hep::rec::LayeredCalorimeterStruct::Layer>& plugLayers= getExtension((
      // dd4hep::DetType::CALORIMETER | dd4hep::DetType::HADRONIC | dd4hep::DetType::AUXILIARY ))->layers;
      UTIL::CellIDDecoder<CalorimeterHit> cellIdDecoder(pCaloHitCollection);
      const std::string layerCodingString(pCaloHitCollection->getParameters().getStringVal(LCIO::CellIDEncoding));
      const std::string layerCoding("layer");
      ...
      for (int i = 0; i < nElements; ++i) {
        try {
          ...
          if (isInBarrelRegion && isWithinCoil) {
            ...
            //                         this->GetEndCapCaloHitProperties(pCaloHit, plugLayers, caloHitParameters,
            //                         absorberCorrection);
...
void DDCaloHitCreator::GetEndCapCaloHitProperties(
  ...
  //     streamlog_out(DEBUG) <<" GetEndCapCaloHitProperties: physLayer: "<<physicalLayer <<" layer:
  //     "<<caloHitParameters.m_layer.Get()<<" nX0: "<<    caloHitParameters.m_nCellRadiationLengths.Get() <<" nLambdaI:
  //     "<<    caloHitParameters.m_nCellInteractionLengths.Get()<<" thickness:
  //     "<<caloHitParameters.m_cellThickness.Get()<<std::endl;
}
...
void DDCaloHitCreator::GetBarrelCaloHitProperties(
  ...
  //     streamlog_out(DEBUG)<<" GetBarrelCaloHitProperties: physLayer: "<<physicalLayer <<" layer:
  //     "<<caloHitParameters.m_layer.Get()<<" nX0: "<<    caloHitParameters.m_nCellRadiationLengths.Get() <<" nLambdaI:
  //     "<<    caloHitParameters.m_nCellInteractionLengths.Get()<<" thickness:
  //     "<<caloHitParameters.m_cellThickness.Get()<<std::endl;
}
...
#endif
  {
    m_caloHitFactory = std::make_unique<
        pandora::PandoraObjectFactory<object_creation::CaloHit::Parameters, object_creation::CaloHit::Object>>();
  }
}
